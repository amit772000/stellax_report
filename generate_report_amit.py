...
def draw_energy_label_impact_table(c,
    ...
    # Lower table baseline (keeps consistent spacing)
    # lower_top = top_y - len(df) * row_h - 44.0

    # # Draw lower background boxes first
    # for i, row in df.iterrows():
    #     y_baseline = int(round(lower_top - i * row_h))
    #     print(y_baseline)
    #     rect_y = y_baseline - box_h / 2.0
    ...
    #     dw = int(row["ΔWWS"])
    #     c.setFillColorRGB(*_box_color_for(dw))
    #     c.rect(col_dvv, rect_y, box_w, box_h, stroke=0, fill=1)

    #     # ΔRent
    #     dr = int(row["ΔRent"])
    #     c.setFillColorRGB(*_box_color_for(dr))
    #     c.rect(col_dmv, rect_y, box_w, box_h, stroke=0, fill=1)

    # # Draw lower table text (vertically centered)
    # c.setFont(font_name, font_size)
    # for i, row in df.iterrows():
    #     # if i < 2:
    #     #     row_h=row_h
    ...
    #     box_center = rect_y + box_h / 2.0
    #     text_baseline = box_center - glyph_half
    #     text_baseline = float(round(text_baseline, 2))

    #     # WWS (right aligned)
    #     c.setFillColorRGB(*gray)
    #     c.drawRightString(col_vv + 10, text_baseline, f"{int(row['WWS'])}")

    #     # ΔWWS centered
    #     c.setFillColorRGB(0, 0, 0)
    #     c.drawCentredString(col_dvv + box_w / 2.0, text_baseline, str(int(row["ΔWWS"])))

    #     # Rent (right aligned)
    #     c.setFillColorRGB(*gray)
    #     c.drawRightString(col_mv + 10, text_baseline, f"€{format_number(row['Rent'])}")

    #     # ΔRent centered (display €0 when zero)
    #     dr = int(row["ΔRent"])
    #     dr_text = f"€{format_short(dr)}" if dr != 0 else "€0"
    #     c.setFillColorRGB(0, 0, 0)
    #     c.drawCentredString(col_dmv + box_w / 2.0, text_baseline, dr_text)
...
def draw_energy_label_page(c, w, h):
    ...
    draw_energy_label_impact_table(
        c,
        top_y=553,      # move up/down to nudge the whole block
        row_h=16,       # consistent spacing (must be >= box_h + padding)
        col_vv=168,
        col_dvv=214,
        col_mv=335,
        col_dmv=422,
...
def draw_ep_online_excerpt_page(c, w, h):
    # Insert the green arrow image (adjust the path and rect as needed)
    # The image should be a transparent PNG with just the arrow shape.
    # We'll overlay the label letter on top.
    # arrow_img_path = "assets/pictures/energy_label_C.png"
    # arrow_rect = fitz.Rect(70, 620, 290, 690)  # (x0, y0, x1, y1) adjust as needed

    # Save the overlay so we can insert the image after PDF is generated
    # c.save()

    # Overlay the energy label letter (centered on the arrow)
    # c.setFont("Bold", 40)
    # c.setFillColorRGB(1, 1, 1)
    # c.drawCentredString(180, 650, variables.energy_label_letter)  # adjust (x, y) as needed

    # Draw right-hand side data
    # c.setFont("Bold", 12)
    # c.setFillColorRGB(0, 0, 0)
    # c.drawString(320, 680, "Register date")
    # c.drawString(320, 660, "Valid till")
    # c.drawString(320, 640, "Energiebehoefte")
    # c.drawString(320, 620, "Certificaathouder")
...
#add this overlay for page 9
filler.add_overlay(9, draw_page10)
filler.add_overlay(10,common_page_template)
# Add overlay for page 12
filler.add_overlay(11, draw_wws_points_page)
# Add this overlay for page 15
filler.add_overlay(15, draw_ep_online_excerpt_page) #actaully 16
...
# After filler.save(...), insert the arrow image using PyMuPDF
insert_image("Template_filled.pdf", "assets/pictures/energy_label_C.png", fitz.Rect(30, 165, 290, 280), page_num=3)
...
insert_image("Template_filled.pdf", rounded_img_10, fitz.Rect(60, 120, 540, 350), page_num=0)
